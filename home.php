...
body {
  /* background: #2a88c5 url(http://zmwlt.com/bucket/cz-20150128-what-color-2-bg.jpg) center center / cover no-repeat; Use 'cover' for background */
  position: relative;
...
#robot {
  ...
  top: 50%;
  left: 50%;
  transform: translate(-245%, -50%); /* Center the robot */
  max-width: 20vw;
  max-height: 80vh;
  margin: 0 auto;
}
...
/* Define hover and other animation variables */
:root {
  --hover-top: -5px;  /* Adjust hover height */
  --hover-bottom: 5px; /* Adjust hover height */
}

@keyframes hover {
  0% {
    transform: translateY(var(--hover-top)); /* Use variables */
  }
  100% {
    transform: translateY(var(--hover-bottom)); /* Use variables */
  }
}
...
@keyframes arms_bounce_left {
  0% {
    transform: translateY(var(--hover-top)) rotate(0deg);
  }
  100% {
    transform: translateY(var(--hover-bottom)) rotate(-10deg);
  }
}
...
.start-practicing:hover {
  ...
  box-shadow: 0 5px 10px rgba(0, 0, 0, 0.2); /* Add a more prominent shadow */
}

.start-practicing:active {
  transform: translateY(0); /* Return to original position on click */
  box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1); /* Slightly smaller shadow on click */
}
...
.start-practicing:hover {
  box-shadow: 0 0 10px rgba(0, 123, 255, 0.5); /* Adjust color and intensity */
}
.img-thumbnail {
    padding: .25rem;
      ...
      <section class="hero-section">
        <div class="container text-center">
            <div class="row align-items-center">
                ...
                <a href="category.php" class="btn btn-primary btn-lg start-practicing" style=" position: relative;top: 599px;    right: 186px;border-radius: 30px;">Start Practicing</a>
...
<section class="hero" style="text-align: center; padding: 30px;">
    <h2 id="hero-title" style="font-size: 2.5rem; font-weight: bold;"></h2> 
    <p>Interviewing for your dream job? Our expert <br>
        AI will generate a personal interview <br>
        with questions based on the job description.</p>
    ...
    <a href="profile.php" class="btn" style="
    ...
    ">Go to Profile</a>
                    ...
                    <img  id="toggle" class="toggle" src="images/power.png" class="img-thumbnail" alt="..."
                    style="    position: relative;
    top: 50px;
    left: 333px;
    height: 47px;" onclick="myFunction()">
                </div>
            </div>
        </div>
    </section>
...
<script>
        const text = "Ace Your Interview with Confidence";
        let i = 0;
        function typeEffect() {
            if (i < text.length) {
                document.getElementById("hero-title").innerHTML += text.charAt(i);
                i++;
                setTimeout(typeEffect, 100); // Speed of typing effect (100ms)
            }
        }
        window.onload = typeEffect; // Run when the page loads
    </script>
</body>
</html>
